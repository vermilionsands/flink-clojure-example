...
(ns flink-example.util.genclass
...
(defn- generate-class [options-map]
  ...
  (let [default-options {:prefix "-" :load-impl-ns true :impl-ns (ns-name *ns*)}
        ...
        class-signature (let [symbols (cons extends implements)]
                          (when (some #(get (meta %) :types) symbols)
...
(defn- generate-interface
  ...
  (let [dname (fn [^Class c] (.. Type (getType c) (getDescriptor)))
        supers (map the-class extends)
        signature (fn [^Class c params]
                    (if (empty? params)
                      (dname c)
                      (. (dname c)
